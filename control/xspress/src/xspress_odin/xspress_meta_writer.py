...
#        self._detector_finished = False  # Require base class to check we have finished
    ...
    def _define_detector_datasets(self):
        ...
        for index in range(self._num_channels):
            ...
            dsets.append(Int32HDF5Dataset(scalar_name, shape=(0, XSPRESS_SCALARS_PER_CHANNEL), maxshape=(None, XSPRESS_SCALARS_PER_CHANNEL), chunks=(4000, 9), rank=2, cache=True, block_size=2000))
            self._logger.info("Adding dataset: {}".format(dtc_name))
            dsets.append(Float64HDF5Dataset(dtc_name, shape=(0,), maxshape=(None,), chunks=(4000,), rank=1, cache=True, block_size=2000))
            self._logger.info("Adding dataset: {}".format(inp_est_name))
            dsets.append(Float64HDF5Dataset(inp_est_name, shape=(0,), maxshape=(None,), chunks=(4000,), rank=1, cache=True, block_size=2000))
